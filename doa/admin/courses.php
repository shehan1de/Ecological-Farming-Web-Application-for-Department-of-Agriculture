...
if (isset($_POST["add_gallery"])) {
    ...
    if (empty($duration)) {
        $durationErr = "Please enter duration";
    }
    if (empty($price)) {
    ...
    if (empty($dateInput)) {
        $dateErr = "Please enter start date";
    }
    ...
    $folder = "images/courses/" . $encryptedProfileLink . "." . $extension;

    if (empty($extension)) {
        $imageErr = "Please select a image";
    }

    $date = date("F j, Y", strtotime($dateInput));
    ...
    if (empty($title)) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO `courses` (`user_id`,`title`, `description`, `date`,`image`,`duration`,`price`) VALUES (?,?, ?, ?,?,?,?)");

        if ($stmt) {

            mysqli_stmt_bind_param($stmt, "sssssss", $userId, $title, $description, $date, $folder, $duration, $price);
                        ...
                        <div class="section-content">
                            <form class="signup-box" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="signup-input one">
                                    <input type="submit" value="Add Course" name="add_gallery">
                                </div>
                            </form>
                        </div>
                    </div>
